...
/**
 * Copy a selected video to the selected/ folder for deployment.
...
function copyToSelectedFolder(slot: string, sourcePath: string): string {
  ...
  // Create a clean filename: slot-name.ext (e.g., "hero.mp4", "industries-healthcare.mp4")
...
export async function GET() {
  ...
  try {
    ...
    const manifest: VideoManifest = JSON.parse(content);
